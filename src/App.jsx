...
export default function App() {
  ...
  useEffect(() => {
    ...
    tle.loadPiAnalystProfile()
      .then((profile) => {
        ...
        piAnalystProfileLoadedRef.current = true;
      })
      .catch(() => {
        if (cancelled) return;
        piAnalystProfileLoadedRef.current = true;
  ...
  useEffect(() => {
    if (!tle?.savePiAnalystProfile || !piAnalystProfileLoadedRef.current) return;
  ...
  const openPiSourceEvent = useCallback(async (rowId) => {
    if (!tle?.getRowsByIds || !ct?.id || !rowId) return;
    setModal((p) => p?.type === "processTree" ? { ...p, ptSourceEventLoading: true, ptSourceEvent: null } : p);
    try {
      const rows = await tle.getRowsByIds(ct.id, [rowId]);
      const row = rows?.[0] || null;
      setModal((p) => p?.type === "processTree"
        ? { ...p, ptSourceEventLoading: false, ptSourceEvent: row ? { rowId, row } : null }
        : p);
    } catch {
  ...
  useEffect(() => {
    if (modal?.type !== "processTree" || modal.phase !== "results" || !tle?.getProcessInspectorContext || !ct?.id) return;
    ...
    const reqKey = `${ct.id}:${selNode.rowid}:${selNode.key}`;
    ...
    tle.getProcessInspectorContext(ct.id, {
      rowId: selNode.rowid,
      selected: {
        rowid: selNode.rowid,
        key: selNode.key,
    ...
    return () => { cancelled = true; };
  }, [ct?.id, modal?.type, modal?.phase, modal?.selectedKey, modal?.data, modal?.ptRelatedEventsKey, modal?.ptRelatedEventsReqKey, tle]);
